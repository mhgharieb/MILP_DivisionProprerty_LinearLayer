...
_logDir = '/tmp/kalyna_4_round_key_dependent_cplex/'
...
def createModel(Rounds, inputDP, filename_model):
    ...
    for ColIndex in range(len(inputDP_column)):
        ...
        if ('0' in currentColDP) and ('1' in currentColDP):
...
if __name__ == "__main__":
    ...
    _col1 = ['1'] * (wordSize * ColSize)
    _col1[InactiveByeIndx] = '0'
    _col1[InactiveByeIndx+1] = '0'
    inputDP = [''.join(_col1[i:i+wordSize]) for i in range(0, len(_col1), wordSize)] + ['0' * wordSize] * ColSize
