...
def words2bits(X):
    # maintain big-endian in bits
    # X = [X0, X1, X2, ....] where X0 is LSB, X0 = [X07, X06, X05, ... , X00] where X00 is lsb
    # return [X07, X06, X05, X04, ... , X17, X16, ...] where X00 is lsb of X0, X10 is lsb of X1 , ....

    return sum(X, [])  

def bytes2bits(X):
    # maintain little-endian in bytes
    # X = [X0, X1, X2, ....] where X0 is LSB, X0 = [X07, X06, X05, ... , X00] where X00 is lsb
    # return [X00, X01, X02, X03, ... , X10, X11, ...] where X00 is lsb of X0, X10 is lsb of X1 , ....
    return sum(X[::-1], [])[::-1]     
...
def ConstraintsBySbox(X, Y, Sbox):
    V = X + Y
    constraints = []
    for inq in Sbox:
...
def ConstraintsBySboxLayer(X, Y):
    constraints = []
    n = len(X)
    for i in range(n):
        constraints += ConstraintsBySbox(X[i], Y[i], SboxInqs[i%4])
    return constraints


#Obtained by ../others/aes-invers_ShiftRow_index.py
def ShiftRow(X):
