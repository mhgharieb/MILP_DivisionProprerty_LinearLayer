...
import time
from AESLike import *
from ModOperations import ConstraintsByModAddConstant
...
def createModel(Rounds, inputDP, filename_model):
    ...
    # Pre-Whitening
    #######################################################################
    P = CreateStateVariable('x', blocksize//wordSize, wordSize, 0)
    PSerialized = words2bits(P)
    InputVariables = PSerialized
    constraints += ['%s = %s' %(a, b) for a,b in zip(PSerialized, inputDP)]

    Xin = CreateStateVariable('x', blocksize//wordSize, wordSize, 0)
    for StateCol in range(len(Xin)//ColSize):
        P_8ByteSlice = P[ColSize*StateCol:ColSize*StateCol+ColSize]
        P_64bitSlice = bytes2bits(P_8ByteSlice)
        Xin_8ByteSlice = Xin[ColSize*StateCol:ColSize*StateCol+ColSize]
        Xin_64bitSlice = bytes2bits(Xin_8ByteSlice)
        tmpVariables , tmpConstraints = ConstraintsByModAddConstant(P_64bitSlice[::-1], Xin_64bitSlice[::-1], StateCol) 
