...
_logDir = '/tmp/photon288_4_round_cplex/'
...
def findAllUNKNOWN(m):
    Z_UNKNOWN = m._XorOutput[m.solution.get_values(m._XorOutput).index(1)]
...
def SubRun(Round, shrunkColIndex, outputBit, Y_Output):
    ...
    for i in range(blocksize//(wordSize*ColSize)):
        if i ==  shrunkColIndex:
            ...
            inputDP_column.append(['1' * wordSize] * ColSize)
    ...
    fileobj.write("shrunkColIndex_%i_outputCheckBit_%i_%s_" %(shrunkColIndex, outputCheckBit, outputBit) + "\n")
    #     # print("====================%02i=====================" %i, flush=True)
        
    logFile = logDir +"shrunkColIndex_%i_outputCheckBit_%i_%s_" %(shrunkColIndex, outputCheckBit, outputBit) +  machine.split('.')[0] + "_" + m._Blockcipher + "_Intgral_%02i_%s" % (Round, str(time.time()))
    ...
    #########cplex parpameter#############
    m.parameters.emphasis.mip.set(1) #CPX_MIPEMPHASIS_FEASIBILITY 
...
if __name__ == "__main__":
    ...
    # if (len(sys.argv) != 6):
    #     print("args sould be 5")
    #     exit()

    # SPEED_JOB = int(sys.argv[1])
    # InputCheckBit_stop = int(sys.argv[2])
    # outputCheckBit_start = int(sys.argv[3])
    # outputCheckBit_end = int(sys.argv[4])
    # InputCheckBit_list = map(int, sys.argv[5].strip('[]').split(','))


    SPEED_JOB = 0
    outputCheckBit_start = 0
    outputCheckBit_end = 288
    ...
    for shrunkColIndex in InputCheckBit_list:
        ...
        filename_TotalResult = "txt/Total_%i_%i.txt" %(shrunkColIndex, SPEED_JOB) 
        ...
        for outputCheckBit in range(outputCheckBit_start, outputCheckBit_end):
            ...
            SubRunCallback(SubRun(Round, shrunkColIndex, outputBit, XorOutput[0]))
